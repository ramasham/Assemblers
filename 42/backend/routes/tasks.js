...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    if (req.user.role === 'technician' && task.technician._id.toString() !== req.user.id) {
...
router.post('/', authenticate, async (req, res) => {
  try {
    const taskData = {
      ...req.body,
      technician: req.user.role === 'technician' ? req.user.id : req.body.technician
    };
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    ...
    // Only allow deletion by supervisor/admin or task owner
    if (req.user.role === 'technician' && task.technician.toString() !== req.user.id) {
      return res.status(403).json({
        success: false,
        message: 'Access denied'
